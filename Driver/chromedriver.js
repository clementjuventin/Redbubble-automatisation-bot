...
class PuppeteerExtra {
    ...
    async connect(options) {
        ...
        // Let's check requirements after plugin had the chance to modify the options
        this.checkPluginRequirements(opts);
        const browser = await require('puppeteer').connect(options);
        this._patchPageCreationMethods(browser);
        await this.callPlugins('_bindBrowserEvents', browser, opts);
        return browser;
    }
...
const redbubble = require(__dirname+'/redbubbleconfig.json');

async function setBrowser() {
    const browserURL = 'http://127.0.0.1:9222';
    browser = await puppeteerExtra.connect({browserURL})
}
...
module.exports = {upload, setBrowser};